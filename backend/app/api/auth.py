...
def _ensure_admin_family(db: Session, user: User) -> None:
    """Auto-create family and egg for admin if they don't have one."""
    ...
    db.add(user)
    db.commit()
    auto_create_egg(db, family.id, user.id)  # type: ignore[arg-type]
...
def register(body: RegisterRequest, db: Session = Depends(get_db)) -> User:
    ...
    db.commit()
    db.refresh(user)
    # Auto-create family and egg on register
...
def login(body: LoginRequest, db: Session = Depends(get_db)) -> TokenResponse:
    ...
    # Ensure admin has family and egg
    _ensure_admin_family(db, user)
